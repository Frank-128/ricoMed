...
function Nurse() {
    ...
    useEffect(()=>{
      setSearchArray(users.filter((item)=> item.name.toLocaleLowerCase().includes(search.toLocaleLowerCase()) && search!==""))
    },[search])
   const handleClick = ()=>{
  ...
  return (
    <div className="w-screen h-screen overflow-y-scroll flex flex-col font-mono">
      <nav className="bg-gray-200 flex justify-between  p-2 z-50 basis-1/12 sticky top-0 left-0">
        ...
        {toggleSearch && <div className="absolute flex items-center flex-col searchResult bg-slate-50 top-20 w-1/4 left-[35%]">
          {searchArray.length !== 0 ? searchArray.map((item,i)=>(<div onClick={handleClick}  className="cursor-pointer p-2 hover:opacity-30">{item.name}</div>)):<div><i>There is nothing to show</i></div>}
        </div>}
